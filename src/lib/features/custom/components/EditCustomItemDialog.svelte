<script lang="ts">
  ...
  let { open = $bindable(false), customItemId }: Props = $props();

  let customItem = $derived(customStore.items.find((i) => i.id === customItemId));
  ...
  $effect(() => {
    if (open && customItem) {
      editTitleValue = customItem.title;
      editContentValue = customItem.content;
      editUrlValue = customItem.url || '';
      editImageUrlValue = customItem.imageUrl || '';
      editTagsList = customItem.tags ? [...customItem.tags] : [];
  ...
  async function confirmEdit() {
    ...
    await customStore.save(customItem.id, {
    ...
    const oldTags = customItem.tags || [];
    for (const tag of oldTags) {
      if (!editTagsList.includes(tag)) await customStore.removeTag(customItem.id, tag);
    }
    for (const tag of editTagsList) {
      if (!oldTags.includes(tag)) await customStore.addTag(customItem.id, tag);
    }
...
<Dialog.Root bind:open>
  <Dialog.Content showCloseButton={false} class="rounded-none border border-border bg-box-bg font-mono text-foreground p-0 gap-0 w-full max-w-[95vw] md:max-w-2xl shadow-xl animate-in fade-in zoom-in-95 duration-150">
    <Dialog.Header class="px-4 pt-4 pb-3 border-b border-border">
      <Dialog.Title class="text-xs font-bold uppercase tracking-widest text-primary">// Edit Item</Dialog.Title>
    </Dialog.Header>
