<script lang="ts">
  ...
  import type { CustomItem } from '../types/custom';
  import { customStore } from '../stores/custom.svelte';
  import {
    getAllUniqueTags,
    getTagSuggestions,
    isNewTagValue,
  ...
  const allTags = $derived(getAllUniqueTags(customStore.items));
  const tagSuggestions = $derived(getTagSuggestions(allTags, customItem.tags, newTagValue));
  const isNewTag = $derived(isNewTagValue(allTags, newTagValue));

  function submitTag(tag: string) {
    const clean = normaliseTag(tag);
    if (clean && !customItem.tags.includes(clean)) {
      onAddTag(customItem.id, clean);
    }
    newTagValue = '';
    addTagOpen = false;
  }
  ...
  const previewImage = $derived(customItem.imageUrl);
</script>
...
<Card.Root class="group relative bg-background border border-transparent flex flex-col min-w-0 py-0 gap-0 shadow-none ring-0 rounded-none">
  ...
  <Card.Header class="p-0 gap-0 min-h-0 block rounded-none border-none">
    <div class="flex items-center justify-between px-2 py-1 border-b border-border text-tui-xs text-muted-foreground select-none shrink-0 w-full">
      <div class="flex items-center gap-1.5 min-w-0">
        ...
        <span class="font-mono text-foreground font-bold truncate">Item</span>
      </div>
      <span class="shrink-0 ml-2 font-mono text-[9px] text-dim-foreground uppercase tracking-widest">customItems</span>
    </div>
  </Card.Header>
  ...
  {#if previewImage}
    <div class="aspect-video w-full overflow-hidden border-b border-border bg-background flex items-center justify-center shrink-0">
      <img src={previewImage} alt={ customItem.title } class="w-full h-full object-cover" />
    </div>
  {:else}
  ...
  <Card.Content class="p-3 flex flex-col flex-1 gap-2 min-h-0 text-xs">
    <div class="min-w-0">
      <h3 class="font-bold text-xs text-foreground leading-tight truncate">{ customItem.title }</h3>
      {#if metaString}
        <p class="text-[9px] text-primary/80 font-mono font-bold uppercase tracking-wider mt-1">{metaString}</p>
      {/if}
      {#if customItem.content}
        <p class="text-tui-xs text-muted-foreground line-clamp-2 mt-1 leading-tight">{ customItem.content }</p>
      {/if}
      
    </div>

    <div class="flex flex-wrap gap-1 items-center mt-auto pt-1">
      {#each customItem.tags as tag (tag)}
        <Badge variant="outline" class="text-tui-2xs px-1 border border-border-dim text-muted-foreground flex items-center gap-1 select-none font-mono">
          *{tag}
          <span onclick={() => onRemoveTag(customItem.id, tag)} class="text-destructive hover:text-red-400 cursor-pointer font-bold text-[8px] ml-0.5" title="Remove tag">x</span>
        </Badge>
      {/each}
    ...
    <div class="flex items-center gap-3 pt-2 border-t border-dashed border-border-dim text-tui-2xs font-bold select-none shrink-0">
      <Button variant="ghost" size="xs" onclick={() => onToggleFavorite(customItem.id)} class="text-muted-foreground hover:text-primary transition-colors uppercase h-auto p-0 bg-transparent hover:bg-transparent font-mono text-tui-2xs font-bold">[{ customItem.isFavorite ? '★ unstar' : '☆ star'}]</Button>
      <Button variant="ghost" size="xs" onclick={() => onEdit(customItem.id)} class="text-muted-foreground hover:text-primary transition-colors uppercase h-auto p-0 bg-transparent hover:bg-transparent font-mono text-tui-2xs font-bold">[edit]</Button>
      <Button variant="ghost" size="xs" onclick={() => onDelete(customItem.id)} class="text-destructive hover:text-red-400 transition-colors uppercase h-auto p-0 bg-transparent hover:bg-transparent font-mono text-tui-2xs font-bold">[del]</Button>
    </div>
  </Card.Content>
</Card.Root>
