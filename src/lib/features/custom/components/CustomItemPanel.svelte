<script lang="ts">
  ...
  import CustomItemGrid from './CustomItemGrid.svelte';
  import CustomItemList from './CustomItemList.svelte';
  import EditCustomItemDialog from './EditCustomItemDialog.svelte';
  ...
  const displayedItems = $derived.by(() => {
    let list = customStore.activeItemsFiltered;
  ...
  async function handleAdd() {
    if (!promptValue.trim() || !activeCategoryId) return;
...
<div class="flex items-center justify-between px-4 py-1.5 border-b border-border bg-box-bg text-xs text-muted-foreground shrink-0 select-none">
  <span class="font-bold uppercase tracking-widest text-foreground">
    [{activeCategory?.icon ?? '?'}] {activeCategory?.name ?? 'Custom Items'}
  </span>
...
<div class="flex-1 overflow-y-auto p-4 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-box-bg [&::-webkit-scrollbar-thumb]:bg-border hover:[&::-webkit-scrollbar-thumb]:bg-primary min-h-0">
  {#if displayedItems.length > 0}
    {#if viewStore.mode === 'list'}
      <CustomItemList customItems={displayedItems} onToggleFavorite={(id) => customStore.toggleFavorite(id)} onDelete={(id) => customStore.softDelete(id)} onAddTag={(id, tag) => customStore.addTag(id, tag)} onRemoveTag={(id, tag) => customStore.removeTag(id, tag)} onEdit={handleEdit} />
    {:else}
      <CustomItemGrid customItems={displayedItems} onToggleFavorite={(id) => customStore.toggleFavorite(id)} onDelete={(id) => customStore.softDelete(id)} onAddTag={(id, tag) => customStore.addTag(id, tag)} onRemoveTag={(id, tag) => customStore.removeTag(id, tag)} onEdit={handleEdit} />
    {/if}
  {:else}
...
{#if editDialogOpen && editId}
  <EditCustomItemDialog bind:open={editDialogOpen} customItemId={editId} />
{/if}
