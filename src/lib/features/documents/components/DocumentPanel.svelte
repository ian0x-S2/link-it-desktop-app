<script lang="ts">
  ...
  const displayedItems = $derived.by(() => {
    let list = documentStore.activeItemsFiltered;
...
<div class="flex items-center justify-between px-4 py-1.5 border-b border-border bg-box-bg text-xs text-muted-foreground shrink-0 select-none">
  <span class="font-bold uppercase tracking-widest text-foreground">
    [{activeCategory?.icon ?? '?'}] {activeCategory?.name ?? 'Documents'}
  </span>
...
<div class="flex-1 overflow-y-auto p-4 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-box-bg [&::-webkit-scrollbar-thumb]:bg-border hover:[&::-webkit-scrollbar-thumb]:bg-primary min-h-0">
  {#if displayedItems.length > 0}
    {#if viewStore.mode === 'list'}
      <DocumentList documents={displayedItems} onToggleFavorite={(id) => documentStore.toggleFavorite(id)} onDelete={(id) => documentStore.softDelete(id)} onAddTag={(id, tag) => documentStore.addTag(id, tag)} onRemoveTag={(id, tag) => documentStore.removeTag(id, tag)} onEdit={handleEdit} />
    {:else}
      <DocumentGrid documents={displayedItems} onToggleFavorite={(id) => documentStore.toggleFavorite(id)} onDelete={(id) => documentStore.softDelete(id)} onAddTag={(id, tag) => documentStore.addTag(id, tag)} onRemoveTag={(id, tag) => documentStore.removeTag(id, tag)} onEdit={handleEdit} />
    {/if}
  {:else}
...
{#if editDialogOpen && editId}
  <EditDocumentDialog bind:open={editDialogOpen} documentId={editId} />
{/if}
