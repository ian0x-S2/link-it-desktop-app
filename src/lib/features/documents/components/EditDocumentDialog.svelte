<script lang="ts">
  ...
  let document = $derived(documentStore.items.find((i) => i.id === documentId));
  ...
  $effect(() => {
    if (open && document) {
      editTitleValue = document.title;
      editContentValue = document.content;
      editUrlValue = document.url || '';
      editImageUrlValue = document.imageUrl || '';
      editTagsList = document.tags ? [...document.tags] : [];
  ...
  async function confirmEdit() {
    ...
    await documentStore.save(document.id, {
    ...
    for (const tag of oldTags) {
      if (!editTagsList.includes(tag)) await documentStore.removeTag(document.id, tag);
    }
    for (const tag of editTagsList) {
      if (!oldTags.includes(tag)) await documentStore.addTag(document.id, tag);
    }
...
<Dialog.Root bind:open>
  <Dialog.Content showCloseButton={false} class="rounded-none border border-border bg-box-bg font-mono text-foreground p-0 gap-0 w-full max-w-[95vw] md:max-w-2xl shadow-xl animate-in fade-in zoom-in-95 duration-150">
    <Dialog.Header class="px-4 pt-4 pb-3 border-b border-border">
      <Dialog.Title class="text-xs font-bold uppercase tracking-widest text-primary">// Edit Document</Dialog.Title>
    </Dialog.Header>
