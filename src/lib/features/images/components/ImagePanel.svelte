...
<div class="flex-1 overflow-y-auto p-4 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-box-bg [&::-webkit-scrollbar-thumb]:bg-border hover:[&::-webkit-scrollbar-thumb]:bg-primary min-h-0">
  {#if displayedItems.length > 0}
    {#if viewStore.mode === 'list'}
      <ImageList images={displayedItems} onToggleFavorite={(id) => imageStore.toggleFavorite(id)} onDelete={(id) => imageStore.softDelete(id)} onAddTag={(id, tag) => imageStore.addTag(id, tag)} onRemoveTag={(id, tag) => imageStore.removeTag(id, tag)} onEdit={handleEdit} />
    {:else}
      <ImageGrid images={displayedItems} onToggleFavorite={(id) => imageStore.toggleFavorite(id)} onDelete={(id) => imageStore.softDelete(id)} onAddTag={(id, tag) => imageStore.addTag(id, tag)} onRemoveTag={(id, tag) => imageStore.removeTag(id, tag)} onEdit={handleEdit} />
    {/if}
  {:else}
...
{#if editDialogOpen && editId}
  <EditImageDialog bind:open={editDialogOpen} imageId={editId} />
{/if}
