<script lang="ts">
  ...
  let { open = $bindable(false), imageId }: Props = $props();

  let image = $derived(imageStore.items.find((i) => i.id === imageId));
  ...
  $effect(() => {
    if (open && image) {
      editTitleValue = image.title;
      editContentValue = image.content;
      editUrlValue = image.url || '';
      editImageUrlValue = image.imageUrl || '';
      editTagsList = image.tags ? [...image.tags] : [];
  ...
  async function confirmEdit() {
    ...
    await imageStore.save(image.id, {
    ...
    for (const tag of oldTags) {
      if (!editTagsList.includes(tag)) await imageStore.removeTag(image.id, tag);
    }
    for (const tag of editTagsList) {
      if (!oldTags.includes(tag)) await imageStore.addTag(image.id, tag);
    }
...
<Dialog.Root bind:open>
  <Dialog.Content showCloseButton={false} class="rounded-none border border-border bg-box-bg font-mono text-foreground p-0 gap-0 w-full max-w-[95vw] md:max-w-2xl shadow-xl animate-in fade-in zoom-in-95 duration-150">
    <Dialog.Header class="px-4 pt-4 pb-3 border-b border-border">
      <Dialog.Title class="text-xs font-bold uppercase tracking-widest text-primary">// Edit Image</Dialog.Title>
    </Dialog.Header>
