<script lang="ts">
  ...
  let { open = $bindable(false), mediaId }: Props = $props();

  let media = $derived(mediaStore.items.find((i) => i.id === mediaId));
  ...
  let statusValue = $state('Plan to Watch');
  ...
  $effect(() => {
    if (open && media) {
      editTitleValue = media.title;
      editContentValue = media.content;
      editUrlValue = media.url || '';
      editImageUrlValue = media.imageUrl || '';
      editTagsList = media.tags ? [...media.tags] : [];
      ratingValue = media.rating || 0;
      statusValue = media.status || 'Plan to Watch';
    }
  });
  ...
  async function confirmEdit() {
    ...
    await mediaStore.save(media.id, {
    ...
    for (const tag of oldTags) {
      if (!editTagsList.includes(tag)) await mediaStore.removeTag(media.id, tag);
    }
    for (const tag of editTagsList) {
      if (!oldTags.includes(tag)) await mediaStore.addTag(media.id, tag);
    }
...
<Dialog.Root bind:open>
  <Dialog.Content showCloseButton={false} class="rounded-none border border-border bg-box-bg font-mono text-foreground p-0 gap-0 w-full max-w-[95vw] md:max-w-2xl shadow-xl animate-in fade-in zoom-in-95 duration-150">
    <Dialog.Header class="px-4 pt-4 pb-3 border-b border-border">
      <Dialog.Title class="text-xs font-bold uppercase tracking-widest text-primary">// Edit Media</Dialog.Title>
    </Dialog.Header>
    ...
    <div class="px-4 py-4 flex flex-col gap-4 overflow-y-auto max-h-[75vh]">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3 border-t border-dashed border-border-dim pt-3">
        <div class="flex flex-col gap-1">
          <label for="edit-rating-media" class="text-tui-2xs uppercase font-bold text-muted-foreground tracking-tui-wide">// Rating</label>
          <select id="edit-rating-media" bind:value={ratingValue} class="bg-box-bg border border-border text-foreground font-mono text-xs px-2 py-1.5 outline-none focus:border-primary">
        ...
        <div class="flex flex-col gap-1">
          <label for="edit-status-media" class="text-tui-2xs uppercase font-bold text-muted-foreground tracking-tui-wide">// Status</label>
          <select id="edit-status-media" bind:value={statusValue} class="bg-box-bg border border-border text-foreground font-mono text-xs px-2 py-1.5 outline-none focus:border-primary">
