<script lang="ts">
  ...
  import AudioGrid from './AudioGrid.svelte';
  import AudioList from './AudioList.svelte';
  import EditAudioDialog from './EditAudioDialog.svelte';
  ...
  const displayedItems = $derived.by(() => {
    let list = audioStore.activeItemsFiltered;
...
<div class="flex items-center justify-between px-4 py-1.5 border-b border-border bg-box-bg text-xs text-muted-foreground shrink-0 select-none">
  <span class="font-bold uppercase tracking-widest text-foreground">
    [{activeCategory?.icon ?? '?'}] {activeCategory?.name ?? 'Audio'}
  </span>
...
<div class="flex-1 overflow-y-auto p-4 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-box-bg [&::-webkit-scrollbar-thumb]:bg-border hover:[&::-webkit-scrollbar-thumb]:bg-primary min-h-0">
  {#if displayedItems.length > 0}
    {#if viewStore.mode === 'list'}
      <AudioList audio={displayedItems} onToggleFavorite={(id) => audioStore.toggleFavorite(id)} onDelete={(id) => audioStore.softDelete(id)} onAddTag={(id, tag) => audioStore.addTag(id, tag)} onRemoveTag={(id, tag) => audioStore.removeTag(id, tag)} onEdit={handleEdit} />
    {:else}
      <AudioGrid audio={displayedItems} onToggleFavorite={(id) => audioStore.toggleFavorite(id)} onDelete={(id) => audioStore.softDelete(id)} onAddTag={(id, tag) => audioStore.addTag(id, tag)} onRemoveTag={(id, tag) => audioStore.removeTag(id, tag)} onEdit={handleEdit} />
    {/if}
  {:else}
...
{#if editDialogOpen && editId}
  <EditAudioDialog bind:open={editDialogOpen} audioId={editId} />
{/if}
