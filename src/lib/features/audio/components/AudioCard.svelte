<script lang="ts">
  ...
  const allTags = $derived(audioStore.allTags);
...
>
  ...
  {#if previewImage}
    ...
    >
      <img src={previewImage} alt={audio.title} loading="lazy" class="w-full h-full object-cover" />
    </div>
  {:else}
    <div
  ...
  <Card.Content class="p-3 flex flex-col flex-1 gap-2 min-h-0 text-xs">
    ...
    >
      <Button
        ...
        >[{audio.isFavorite ? '★ unstar' : '☆ star'}]</Button
      >
      <Button
        variant="ghost"
        size="xs"
