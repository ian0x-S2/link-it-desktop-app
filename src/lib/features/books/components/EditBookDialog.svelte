<script lang="ts">
  ...
  $effect(() => {
    if (open && book) {
      ...
      statusValue = book.status || 'Backlog';
    }
  });
...
<Dialog.Root bind:open>
  <Dialog.Content showCloseButton={false} class="rounded-none border border-border bg-box-bg font-mono text-foreground p-0 gap-0 w-full max-w-[95vw] md:max-w-2xl shadow-xl animate-in fade-in zoom-in-95 duration-150">
    <Dialog.Header class="px-4 pt-4 pb-3 border-b border-border">
      <Dialog.Title class="text-xs font-bold uppercase tracking-widest text-primary">// Edit Book</Dialog.Title>
    </Dialog.Header>
    ...
    <div class="px-4 py-4 flex flex-col gap-4 overflow-y-auto max-h-[75vh]">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-3 border-t border-dashed border-border-dim pt-3">
        <div class="flex flex-col gap-1">
          <label for="edit-author" class="text-tui-2xs uppercase font-bold text-muted-foreground tracking-tui-wide">// Author</label>
          <div class="flex items-center gap-1.5 px-2.5 py-1.5 border border-border bg-transparent">
            <Input id="edit-author" bind:value={authorValue} class="flex-1 bg-transparent border-none outline-none text-foreground font-mono text-xs h-auto py-0 focus-visible:ring-0" />
          </div>
        </div>
        <div class="flex flex-col gap-1">
          <label for="edit-rating" class="text-tui-2xs uppercase font-bold text-muted-foreground tracking-tui-wide">// Rating</label>
          <select id="edit-rating" bind:value={ratingValue} class="bg-box-bg border border-border text-foreground font-mono text-xs px-2 py-1.5 outline-none focus:border-primary">
        ...
        <div class="flex flex-col gap-1">
          <label for="edit-status" class="text-tui-2xs uppercase font-bold text-muted-foreground tracking-tui-wide">// Status</label>
          <select id="edit-status" bind:value={statusValue} class="bg-box-bg border border-border text-foreground font-mono text-xs px-2 py-1.5 outline-none focus:border-primary">
