...
<div class="group relative flex flex-col md:flex-row md:items-center justify-between p-3 border border-border bg-background gap-3 font-mono text-xs hover:border-primary transition-colors rounded-none">
  <div class="flex items-start gap-3 min-w-0 flex-1">
    <div class="flex items-center gap-1.5 shrink-0 select-none text-muted-foreground mt-0.5">
      ...
      <span onclick={() => onToggleFavorite(book.id)} class="cursor-pointer hover:text-primary transition-colors font-bold select-none">{ book.isFavorite ? '★' : '☆'}</span>
    </div>
    ...
    <div class="flex-1 min-w-0">
      <div class="flex flex-col md:flex-row md:items-baseline gap-x-2 gap-y-0.5">
        <!-- svelte-ignore a11y_click_events_have_key_events -->
        <!-- svelte-ignore a11y_no_static_element_interactions -->
        <span onclick={() => onEdit(book.id)} class="font-bold text-foreground hover:text-primary transition-colors cursor-pointer truncate">{ book.title }</span>
        {#if book.url}
          <a href={ book.url } target="_blank" rel="noopener noreferrer" class="text-tui-2xs text-muted-foreground hover:text-primary truncate font-normal">({ book.url })</a>
        {/if}
      </div>
